...
class image_dehazer():
    ...
    def __BoundCon(self, HazeImg):
        if (len(HazeImg.shape) == 3):
            ...
            t_b = np.maximum((self._A[0] - HazeImg[:, :, 0].astype(np.float32)) / (self._A[0] - self.C0),
                             (HazeImg[:, :, 0].astype(np.float32) - self._A[0]) / (self.C1 - self._A[0]))
            t_g = np.maximum((self._A[1] - HazeImg[:, :, 1].astype(np.float32)) / (self._A[1] - self.C0),
                             (HazeImg[:, :, 1].astype(np.float32) - self._A[1]) / (self.C1 - self._A[1]))
            t_r = np.maximum((self._A[2] - HazeImg[:, :, 2].astype(np.float32)) / (self._A[2] - self.C0),
            ...
            self._Transmission = np.maximum((self._A[0] - HazeImg.astype(np.float32)) / (self._A[0] - self.C0),
                                            (HazeImg.astype(np.float32) - self._A[0]) / (self.C1 - self._A[0]))
            self._Transmission = np.minimum(self._Transmission, 1)

        kernel = np.ones((self.boundaryConstraint_windowSze, self.boundaryConstraint_windowSze), np.float32)
    ...
    def __psf2otf(self, psf, shape):
        ...
        # roundoff error  = machine epsilon = sys.np.float32_info.epsilon
        # or np.finfo().eps
